...
> `.env.local.example` にテンプレートが用意されています。`cp .env.local.example .env.local` でコピーして使ってください。
...
> - `NEXT_PUBLIC_PROTO_PASSWORD` を設定してパスワード保護を追加する
...
- **~~パスワード保護~~** — 実装済み。`.env.local` で `NEXT_PUBLIC_PROTO_PASSWORD` を設定するだけ
