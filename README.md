...
テンプレートの基本機能に加えて、こんな拡張ができます。
すべて Claude Code に「〜を追加して」と頼むだけで実装できます。

### デザインプロセスの強化

| アイデア | 説明 |
|---------|------|
| **Figma 連携** | Figma MCP を使えば、既存の Figma デザインを読み込んでコードに変換。逆にプロトのスクショを Figma に戻すことも可能 |
| **コメント機能** | [Liveblocks](https://liveblocks.io/) + [Supabase](https://supabase.com/) で、プロトタイプ上に直接コメントをピン留め（詳細は下記） |
| **デザインレビュー AI** | スクリーンショットを Claude に見せて「この画面の UX を改善して」— AI がレビュアーになる |
| **レスポンシブ自動キャプチャ** | Playwright のスクショスクリプトを拡張して、モバイル / タブレット / デスクトップの 3 サイズで一括撮影 |

### プロトタイプの品質向上

| アイデア | 説明 |
|---------|------|
| **shadcn/ui 導入** | `npx shadcn@latest init` で本格的な UI コンポーネントを追加。ボタン、ダイアログ、フォームなどが即使える |
| **ダークモード** | `globals.css` にダーク用トークンを追加し、`prefers-color-scheme` で切り替え |
| **アニメーション** | Framer Motion を追加して、ページ遷移やマイクロインタラクションをプロトタイプ |
| **実データ接続** | Supabase や REST API に接続して、実際のデータでプロトタイプを動かす |
...
| **Slack 通知** | デプロイ完了時に Slack チャンネルへ URL を自動投稿 |
| **フィードバック収集** | プロトタイプにフィードバックフォームを埋め込み、Notion や Google Sheets に記録 |
| **スキルの共有** | チーム共通の `SKILL.md` を Git で管理し、全員が同じワークフローで作業 |
| **パスワード保護** | 機密性の高いプロトタイプに簡易認証を追加（環境変数でパスワード管理） |
