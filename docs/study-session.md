...
**3つの軸で画面バリエーションを管理**:
| 概念 | 誰が切り替える？ | 本番に残る？ | クエリパラム | 色 |
|------|-----------------|------------|-------------|-----|
| **State** | ユーザー（アプリ内UI操作） | 全て残る | `_tab=xxx` | グレー |
| **Variant** | システム（データ条件） | 全て残る | `_v=xxx` | オレンジ |
| **Pattern** | デザイナー（設計方針の比較） | 1つだけ採用 | `_p=xxx` | バイオレット |

**State vs Pattern の判断基準**:
アプリ内にトグルUI（切替ボタン・タブ等）があるものは **State**（例: グリッド/リスト表示切替）。
Pattern はアプリ内にトグルUIを持たず、Compare パネルからのみ切り替える（例: カード型 vs タイムライン型レビュー）。

スクリーンショットの自動撮影（Playwright）で、全パターンを一括キャプチャ。ProtoNav や Next.js 開発UIは撮影前に自動非表示。
ProtoNav 右端の **Compare (N)** ボタンで、ページ遷移なしに Compare ドロワーを開いて横並び比較。ドロワーはページ遷移に自動追従。
...
│   ├── page.tsx             # カフェ一覧（Cafe Explorer サンプル）
│   ├── cafe/page.tsx        # カフェ詳細（メニュー・レビュー）
│   ├── proto-nav.tsx        # ナビバー + Compare ボタン + ScreenPanel ドロワー
│   ├── base-path.ts         # GitHub Pages basePath ユーティリティ
│   ├── map/
│   │   ├── screens.ts      # 画面定義（中央設定 / State・Variant・Pattern）
│   │   ├── page.tsx         # React Flow キャンバス
│   │   ├── screen-node.tsx  # ノード表示コンポーネント
│   │   └── screen-panel.tsx # サイドパネル（Map + Compare で共有）
│   ├── spec/page.tsx        # 実装指示書（サンプル記入済み）
│   └── qa/page.tsx          # QA チェックリスト（サンプル記入済み）
├── scripts/
│   └── capture-screens.ts   # Playwright スクショ自動撮影（非アプリUI自動非表示）
